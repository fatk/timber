...
class Pagination
{
    ...
    protected function init($prefs = [], $wp_query = null)
    {
        ...
        // Build prev link directly if current > 1
        if ($this->current > 1) {
            $prevPage = $this->current - 1;
            $link = \str_replace('%_%', 1 == $prevPage ? '' : $args['format'], (string) $args['base']);
            $link = \str_replace('%#%', $prevPage, $link);
        ...
        // Build next link directly if current < total
        if ($this->current < $args['total']) {
            $nextPage = $this->current + 1;
            $link = \str_replace('%_%', $args['format'], (string) $args['base']);
            $link = \str_replace('%#%', $nextPage, $link);
            $link = URLHelper::user_trailingslashit($link);
            if (!empty($args['add_args'])) {
                $link = \add_query_arg($args['add_args'], $link);
            }
